...
for j in range(len(data)):
	for i in range(len(data[0])-1):
		if (data[j][i][0] > data[j][i+1][0]):
			edge = [data[j][i],data[j][i+1]]
			edges.append(edge)
		elif data[j][i][0] < data[j][i+1][0]:
			edge = [data[j][i+1],data[j][i]]
			edges.append(edge)
		elif data[j][i][1] > data[j][i+1][1]:
			edge = [data[j][i],data[j][i+1]]
			edges.append(edge)
		elif data[j][i][1] < data[j][i+1][1]:
			edge = [data[j][i+1],data[j][i]]
			edges.append(edge)
		elif data[j][i][2] > data[j][i+1][2]:
			edge = [data[j][i],data[j][i+1]]
			edges.append(edge)
		elif data[j][i][2] < data[j][i+1][2]:
			edge = [data[j][i+1],data[j][i]]
...
ax.set_xlabel('ubar')
ax.set_ylabel('dbar')
ax.set_zlabel('gluons')
for i in range(len(edgy_edges)):
	ax.plot([edgy_edges[i][0][0],edgy_edges[i][1][0]],[edgy_edges[i][0][1],edgy_edges[i][1][1]],([edgy_edges[i][0][2],edgy_edges[i][1][2]]), c = (edgy_colors[i]), linewidth = 3)
plt.show()
